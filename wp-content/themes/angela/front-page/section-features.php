<div class="front_page_section front_page_section_features<?php
			$angela_scheme = angela_get_theme_option('front_page_features_scheme');
			if (!angela_is_inherit($angela_scheme)) echo ' scheme_'.esc_attr($angela_scheme);
			echo ' front_page_section_paddings_'.esc_attr(angela_get_theme_option('front_page_features_paddings'));
		?>"<?php
		$angela_css = '';
		$angela_bg_image = angela_get_theme_option('front_page_features_bg_image');
...
?>><?php
	// Add anchor
	$angela_anchor_icon = angela_get_theme_option('front_page_features_anchor_icon');	
	$angela_anchor_text = angela_get_theme_option('front_page_features_anchor_text');	
	...
	<div class="front_page_section_inner front_page_section_features_inner<?php
			if (angela_get_theme_option('front_page_features_fullheight'))
				echo ' angela-full-height sc_layouts_flex sc_layouts_columns_middle';
			?>"<?php
			$angela_css = '';
			$angela_bg_mask = angela_get_theme_option('front_page_features_bg_mask');
			$angela_bg_color = angela_get_theme_option('front_page_features_bg_color');
	...
	?>>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$angela_caption = angela_get_theme_option('front_page_features_caption');
			if (!empty($angela_caption) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo !empty($angela_caption) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post($angela_caption); ?></h2><?php
			}
		
			// Description (text)
			$angela_description = angela_get_theme_option('front_page_features_description');
			if (!empty($angela_description) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo !empty($angela_description) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post(wpautop($angela_description)); ?></div><?php
			}
			...
			?><div class="front_page_section_output front_page_section_features_output"><?php 
				if (is_active_sidebar('front_page_features_widgets')) {
					...
					if (!angela_exists_trx_addons())
						...
						angela_customizer_need_widgets_message('front_page_features_caption', 'ThemeREX Addons - Services');
				}
			?></div>
		</div>
	</div>
