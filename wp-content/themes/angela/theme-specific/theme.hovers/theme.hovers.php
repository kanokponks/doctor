...
if (!function_exists('angela_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'angela_hovers_theme_setup3', 3 );
	function angela_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		angela_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'angela'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'angela') ),
				"std" => 'slide_left',
				"options" => array(
					'default'		=> esc_html__('Fade',				'angela'),
					'slide_left'	=> esc_html__('Slide from Left',	'angela'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'angela'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'angela') ),
				"std" => 'icon',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'angela'),
					'icon'	=> esc_html__('Icon',	'angela'),
					'icons'	=> esc_html__('Icons',	'angela'),
					'zoom'	=> esc_html__('Zoom',	'angela'),
					'fade'	=> esc_html__('Fade',	'angela'),
					'slide'	=> esc_html__('Slide',	'angela'),
					'pull'	=> esc_html__('Pull',	'angela'),
					'border'=> esc_html__('Border',	'angela')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('angela_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'angela_hovers_theme_setup9', 9 );
	function angela_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'angela_hovers_frontend_scripts', 1010 );
		add_filter( 'angela_filter_localize_script','angela_hovers_localize_script' );
		add_filter( 'angela_filter_merge_scripts',	'angela_hovers_merge_scripts' );
		add_filter( 'angela_filter_merge_styles',	'angela_hovers_merge_styles' );
		add_filter( 'angela_filter_get_css', 		'angela_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'angela_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'angela_hovers_frontend_scripts', 1010 );
	function angela_hovers_frontend_scripts() {
		if ( angela_is_on(angela_get_theme_option('debug_mode')) && angela_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'angela-hovers', angela_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( angela_is_on(angela_get_theme_option('debug_mode')) && angela_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'angela-hovers',  angela_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('angela_hovers_merge_scripts')) {
	//Handler of the add_filter( 'angela_filter_merge_scripts', 'angela_hovers_merge_scripts' );
	function angela_hovers_merge_scripts($list) {
...
if (!function_exists('angela_hovers_merge_styles')) {
	//Handler of the add_filter( 'angela_filter_merge_styles', 'angela_hovers_merge_styles' );
	function angela_hovers_merge_styles($list) {
...
if (!function_exists('angela_hovers_localize_script')) {
	//Handler of the add_filter( 'angela_filter_localize_script','angela_hovers_localize_script' );
	function angela_hovers_localize_script($arr) {
		$arr['button_hover'] = angela_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('angela_hovers_add_icons') ) {
	function angela_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
				...
				if (!is_object($args['cat']) && $product->is_purchasable() && $product->is_in_stock()) {
					angela_show_layout(apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'angela') : '')
										. '</a>',
										$product));
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'angela');
						else
							esc_html_e('Details', 'angela');
					}
				?></a>
			</div>
			<?php
		...
		// Hover style 'Icon'
			...
			?><div class="icons"><a href="<?php the_permalink(); ?>" aria-hidden="true" class="icon-view"></a></div><?php
...
// Add styles into CSS
if ( !function_exists( 'angela_hovers_get_css' ) ) {
	//Handler of the add_filter( 'angela_filter_get_css', 'angela_hovers_get_css', 10, 4 );
	function angela_hovers_get_css($css, $colors, $fonts, $scheme='') {
...
/* Slide */
.sc_button_hover_slide_left {	background: linear-gradient(to right,	{$colors['text_hover2']} 0%, {$colors['text_link2']} 50%, {$colors['text_link2']} 50%, {$colors['text_hover2']} 100%) no-repeat scroll right bottom / 210% 100% {$colors['text_link']} !important; }
