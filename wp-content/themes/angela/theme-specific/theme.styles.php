...
if (!function_exists('angela_customizer_get_css')) {

	function angela_customizer_get_css($colors=null, $fonts=null, $remove_spaces=true, $only_scheme='') {
		...
		if ($fonts) {
			...
			$fonts = angela_customizer_add_theme_fonts($fonts);
...
CSS;
			$rez = apply_filters('angela_filter_get_css', $rez, false, $fonts, '');
			$css['fonts'] = $rez['fonts'];

			
			// Border radius
			//--------------------------------------
			$rad = angela_get_border_radius();
			$rad50 = ' '.$rad != ' 0' ? '50%' : 0;
			$css['fonts'] .= <<<CSS
...
CSS;
		...
		//--------------------------------------
		if ($colors !== false) {
			$schemes = empty($only_scheme) ? array_keys(angela_get_list_schemes()) : array($only_scheme);
	
			if (count($schemes) > 0) {
				$rez = array();
				foreach ($schemes as $scheme) {
					// Prepare colors
					if (empty($only_scheme)) $colors = angela_get_scheme_colors($scheme);
...
.angela_tabs .angela_tabs_titles li a:hover {
	color: {$colors['inverse_link']};
	background-color: {$colors['text_link']};
}
div.esg-filter-wrapper .esg-filterbutton.selected > span,
.mptt-navigation-tabs li.active a,
.angela_tabs .angela_tabs_titles li.ui-state-active a {
	color: {$colors['bg_color']};
	background-color: {$colors['text_dark']};
}
...
.post_date a:hover:before,
a.post_counters_item:hover,
a.post_counters_item:hover:before,
...
.scheme_self.sidebar .sidebar_inner .widget .widget_title a,
.scheme_self.sidebar .sidebar_inner .widget .widget_title,
...
.scheme_self.sidebar .sidebar_inner .widget.widget_search input[type="search"] {
    background-color: {$colors['bg_color_02']};
    border-color: {$colors['bg_color_0']};
    color: {$colors['inverse_link']};
}
.scheme_self.sidebar .sidebar_inner .widget.widget_search form:after {
    color: {$colors['inverse_link']};
}
.scheme_self.sidebar .sidebar_inner .widget.widget_search form:hover:after {
    color: {$colors['bg_color_07']};
}
.scheme_self.sidebar .sidebar_inner .widget.widget_search input[type="search"]::-webkit-input-placeholder {
    color: {$colors['inverse_link']};
}
...
.scheme_self.footer_wrap a.social_item:hover,
.footer_wrap .scheme_self.vc_row a.social_item:hover {
	color: {$colors['inverse_link']};
}
.footer_logo_inner {
	border-color: {$colors['alter_bd_color']};
...
CSS;
					...
					$rez = apply_filters('angela_filter_get_css', $rez, $colors, false, $scheme);
		...
		return apply_filters( 'angela_filter_prepare_css', $css_str, $remove_spaces );
	}
}
?>